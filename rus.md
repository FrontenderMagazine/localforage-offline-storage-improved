...
У веб-приложений есть возможности для автономной работы, например, возможность хранить большие наборы
данных и бинарных файлов на протяжении некоторого времени. Можно даже 
[кешировать MP3 файлы][1]. Браузерные технологии умеют хранить данные в офлайн-режиме, причем в большом количестве.  Однако проблема в том, что выбор технологий 
...
асинхронные, быстрые и поддерживают большие наборы данных, однако имеют 
довольно запутанные API. Также `IndexedDB`, ни `WebSQL` поддерживаются
не во всех современных браузерах и не похоже на то, что в 
ближайшем будущем это изменится.

Если вам нужно написать веб-приложение с поддержкой работы в оффлайн, и вы не 
...
[localForage][3], которая намного упрощает хранение данных в автономном режиме , в *любом* браузере.

Написанный мной клиент Foursquare на [HTML5][5] под названием [around][4], помог 
мне решить проблемные вопросы офлайн-хранения данных. Мы точно так же 
разберёмся с использованием localForage, однако для тех, кто предпочитает 
учиться посредством разбора кода, уже есть пример для изучения.

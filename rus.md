...
У веб-приложений есть автономные возможности, такие как хранение больших наборов 
данных и бинарных файлов на протяжении некоторого времени. Можно даже 
[кешировать MP3 файлы][1]. Браузерные технологии умеют хранить данные в офлайн 
режиме, причем в большом количестве. Проблема однако в том, что выбор технологий 
для этого фрагментирован.

`localStorage` предоставляет возможность хранения данных, однако оно работает 
медленно и не подходит для бинарных блобов. [IndexedDB][2] и `WebSQL` 
асинхронные, быстрые и поддерживают большие наборы данных, однако их API 
довольно запутанные. Тем не менее, ни `IndexedDB`, ни `WebSQL` не имеют 
поддержки всех современных производителей браузеров и не похоже на то что в 
ближайшем будущем это изменится.

Если вам нужно написать веб-приложение с поддержкой автономной работы и вы не 
знаете с чего начать, эта статья для вас. Если вы когда-либо пробовали работать 
с поддержкой офлайн-режима, но у вас от этого только голова шла кругом, эта 
статья также для вас. Компания Mozilla создала библиотеку под названием 
[localForage][3], которая превращает хранение данных в автономном режиме в 
*любом* браузере в намного более простую задачу.

Написанный мной клиент Foursquare на [HTML5][5] под названием [around][4], помог 
мне преодолеть проблемные вопросы офлайн-хранения данных. Мы точно так же 
разберёмся с использованием localForage, однако для тех, кто предпочитает 
учиться посредством разбора кода, уже есть источник изучения.
...
Традиционный `localStorage` API во многих отношениях на деле очень хорош; он 
...
приложения. Особенно плохо это сказывается на мобильных устройствах; главный 
поток остановлен пока не будут извлечены данные, приложение кажется медленным и 
даже перестает реагировать на действия пользователя.
...
localForage позволяет решить эти две проблемы посредством использования 
...
15+). При этом базовый Android Browser (2.1+) и Safari используют `WebSQL`.
...
намного медленнее). Он хотя бы заботится об автоматической конвертации данных 
из/в строки JSON, которые нужны localStorage для хранения данных.

[Узнайте больше о localForage на GitHub][12] и пожалуйста создайте issue если 
